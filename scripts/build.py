...
# ---------------------------------------------------------------- README
def social_badges(cfg, write):
    spec = {"linkedin": "LinkedIn", "x": "X", "email": "Email", "instagram": "Instagram", "youtube": "YouTube", "website": "Website"}
    out = []
    for k, v in cfg.get("social", {}).items():
        if k not in spec or not v:
            continue
        href = f"mailto:{v}" if k == "email" else ("https://x.com/" + v.lstrip("@") if k == "x" and not v.startswith("http") else v)
        write(f"social-{k}.svg", art.social_badge(spec[k], k))
        out.append(f'<a href="{href}"><img src="./assets/social-{k}.svg" alt="{spec[k]}" height="28"></a>')
    return "\n&nbsp;&nbsp;\n".join(out)
